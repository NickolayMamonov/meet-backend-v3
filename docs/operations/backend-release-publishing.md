...
2. `materialize`: only a newly admitted current-action draft with exact tag,
   version, source SHA, numeric release ID, absent `v1.2.0` ref, and empty
   asset inventory.
...
builds one `linux/amd64` image with only `v1.2.0`, `1.2.0`, and
...
explicit tag `v1.2.0`, positive numeric release ID, and source SHA. It fetches
the numeric release, resolves `v1.2.0` back to the same ID, requires
`immutable=true`, and verifies the automatic release attestation. It invokes
each asset check exactly as:

```text
gh release verify-asset v1.2.0 <asset-path> --repo nickolaymamonov/meet-backend-v3
```

An omitted or wrong tag, or a latest release that resolves to `v1.0.1`, cannot
pass. The normalized proof binds repository, tag, release ID, source,
...
checksummed `MEE2-48-v1.2.0-immutability-proof` artifact is the only
repository-owned control-plane write after PATCH.

## Terminal partial and rollback

If any external v1.2.0 object exists and the run stops, do not rerun it. Leave
...
then open separately authorized incident work. Before any v1.2.0 external
