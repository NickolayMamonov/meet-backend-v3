...
class MeetingEndTimeMigrationPostgresTest : IntegrationTestSupport() {
    ...
    @Test
    fun `v8 adds nullable bigint ends_at without changing legacy rows`() {
        ...
        val legacy = meetings.saveAndFlush(meeting("legacy", endsAt = null))
        val explicit = meetings.saveAndFlush(meeting("explicit", endsAt = 1_800_000_000_000))

        assertEquals(null, meetings.findById(legacy.id!!).orElseThrow().endsAt)
        assertEquals(1_800_000_000_000, meetings.findById(explicit.id!!).orElseThrow().endsAt)
    }
