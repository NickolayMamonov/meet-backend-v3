...
for name in protected-v1.0.1 protected-v1.1.0-draft protected-v1.2.0 \
  protected-v1.3.0 test-candidate; do
  run_case "$name"
done
...
jq -e '
  ...
  ($all[] | select(.name == "protected-v1.1.0-draft")) as $v110 |
  ($all[] | select(.name == "protected-v1.2.0")) as $v120 |
  ($all[] | select(.name == "protected-v1.3.0")) as $v130 |
  ...
  ($v110 | .releaseId == 368531227 and .tag == "v1.1.0" and
    .version == "1.1.0" and
    .source == "36ffd11ea4d35147f1df9c1cafa6a330300c1339" and
    .root == "sha256:c156a8a1436b008eea2980711b233b6f800cf60a36cdbe08faf480a2c97e6570") and
  ($v130 | .releaseId == 377201468 and .tag == "v1.3.0" and
    .version == "1.3.0" and
    .source == "a7abfe04f6852f479291a4710ebdee23e9ae8a34" and
    .root == "sha256:88b697872331ed2786f2d9009c769a87c32470086702faacf9874576fe094e9e") and
...
v110=$("$RESOLVER" protected-release \
  --release-id 368531227 --tag v1.1.0 --version 1.1.0 \
  --source "$v110_source" --root "$v110_root" --platform "$platform")
jq -e '
  .certificateSourceDigest == "8598a31e60d0bae784bebf43404f3b1e91d603e1" and
  .signerDigest == .certificateSourceDigest and
  .evidenceStorage == {kind:"oci-registry-bundle"} and
  .subject.name == "ghcr.io/nickolaymamonov/meet-backend-v3"
' <<<"$v110" >/dev/null || fail "v1.1.0 draft authority is incomplete"
...
v130=$("$RESOLVER" protected-release \
  --release-id 377201468 --tag v1.3.0 --version 1.3.0 \
  --source "$v130_source" --root "$v130_root" --platform "$platform")
jq -e '
  .certificateSourceDigest == "79263bfed6427dc1a45900e338805c52dbd5f59d" and
  .signerDigest == .certificateSourceDigest and
  .evidenceStorage.kind == "github-api-workflow-artifact" and
  .evidenceStorage.bundleDigest ==
    "sha256:cb48b0ad1cf2a02733057e337bbba3ea4dc493c311f95117afb3381f894e19a8" and
  (.evidenceStorage.assets | length == 4) and
  .evidenceStorage.asset.name == "image-index.json" and
  .evidenceStorage.asset.id == 532339069 and
  .evidenceStorage.asset.apiDigest == .rootDigest and
  .evidenceStorage.asset.downloadSha256 == (.rootDigest | sub("^sha256:";""))
' <<<"$v130" >/dev/null || fail "v1.3.0 exact asset authority is incomplete"
...
echo "image attestation authority fixtures passed: exact v1.0.1/v1.1.0/v1.2.0/v1.3.0 tuples, candidate closure, assets, sorted output, and mutation rejection"
