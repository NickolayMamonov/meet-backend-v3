...
if [ "$#" -eq 0 ]; then
  cat >/dev/null
  if [[ "${FAKE_GH_RESPONSE:-}" == */v130-response.json ]]; then
    printf '%s  -\n' cb48b0ad1cf2a02733057e337bbba3ea4dc493c311f95117afb3381f894e19a8
  else
    printf '%s  -\n' cf1f5d905c0bb97ca2013b3dd8aa415fb331a63dfec860e0382e5690339e5958
  fi
  exit 0
fi
if [[ "$1" == */v130/image-index.json ]]; then
  digest=88b697872331ed2786f2d9009c769a87c32470086702faacf9874576fe094e9e
else
  case "$(basename -- "$1")" in
    v101-index.json)
      digest=41be6a4e725898bf41823a66abc78dc19f11f31282a3ad574298729095ba59c6 ;;
    v110-index.json)
      digest=c156a8a1436b008eea2980711b233b6f800cf60a36cdbe08faf480a2c97e6570 ;;
    image-index.json)
...
printf 'fixture v1.1.0 index\n' >"$TMP/v110-index.json"
printf 'fixture v1.2.0 index\n' >"$TMP/image-index.json"
mkdir "$TMP/v130"
printf 'fixture v1.3.0 index\n' >"$TMP/v130/image-index.json"
...
make_proof() {
  local version=$1 release_id=$2 source=$3 assets=$4 output=$5
  jq -cnS --arg version "$version" --argjson release_id "$release_id" \
    --arg source "$source" --argjson assets "$assets" '
    {repository:"NickolayMamonov/meet-backend-v3",tag:("v"+$version),
     ...
     assets:[$assets[] + {repository:"NickolayMamonov/meet-backend-v3",
       tag:("v"+$version),releaseId:$release_id,
...
V110_AUTH=$TMP/v110-authority.json
V120_AUTH=$TMP/v120-authority.json
V130_AUTH=$TMP/v130-authority.json
...
make_authority protected-release 368531227 v1.1.0 1.1.0 "$V110_SOURCE" \
  "$V110_SIGNER" "$V110_SIGNER" .github/workflows/release-please.yml \
  "$V110_ROOT" "$IMAGE" oci-registry-bundle "$V110_AUTH"
...
make_authority protected-release 377201468 v1.3.0 1.3.0 "$V130_SOURCE" \
  "$V130_SIGNER" "$V130_SIGNER" .github/workflows/release-please.yml \
  "$V130_ROOT" image-index.json github-api-workflow-artifact "$V130_AUTH"
...
V110_RESPONSE=$TMP/v110-response.json
V120_RESPONSE=$TMP/v120-response.json
V130_RESPONSE=$TMP/v130-response.json
CANDIDATE_RESPONSE=$TMP/candidate-response.json
make_verified "$V101_AUTH" "$V101_RESPONSE"
make_verified "$V110_AUTH" "$V110_RESPONSE"
make_verified "$V120_AUTH" "$V120_RESPONSE"
make_verified "$V130_AUTH" "$V130_RESPONSE"
make_verified "$CANDIDATE_AUTH" "$CANDIDATE_RESPONSE"
V101_CLOSURE=$TMP/v101-closure.json
V110_CLOSURE=$TMP/v110-closure.json
CANDIDATE_CLOSURE=$TMP/candidate-closure.json
make_closure "$V101_CLOSURE"
cp "$V101_CLOSURE" "$V110_CLOSURE"
cp "$V101_CLOSURE" "$CANDIDATE_CLOSURE"
V120_PROOF=$TMP/v120-proof.json
V130_PROOF=$TMP/v130-proof.json
make_proof 1.2.0 371012814 "$V120_SOURCE" "$(assets_json)" "$V120_PROOF"
make_proof 1.3.0 377201468 "$V130_SOURCE" \
  '[{"id":532339115,"name":"SHA256SUMS","size":255,"apiDigest":"sha256:ac70335a2301856b09a912400b977cb4d67653c9d7a370ad5834ae5f591dd476","downloadSha256":"ac70335a2301856b09a912400b977cb4d67653c9d7a370ad5834ae5f591dd476"},{"id":532339069,"name":"image-index.json","size":857,"apiDigest":"sha256:88b697872331ed2786f2d9009c769a87c32470086702faacf9874576fe094e9e","downloadSha256":"88b697872331ed2786f2d9009c769a87c32470086702faacf9874576fe094e9e"},{"id":532339092,"name":"image-inspect.txt","size":849,"apiDigest":"sha256:5f0836e04868ca8e036d44a8c5517d84dfc51bbf1282a87a589d3acd2443addf","downloadSha256":"5f0836e04868ca8e036d44a8c5517d84dfc51bbf1282a87a589d3acd2443addf"},{"id":532339036,"name":"release-manifest.json","size":695,"apiDigest":"sha256:cd34df22ab90a6d5f5b3624bed03d7c2527a842e278136b383bcebe985f1ca6e","downloadSha256":"cd34df22ab90a6d5f5b3624bed03d7c2527a842e278136b383bcebe985f1ca6e"}]' \
  "$V130_PROOF"
...
run_case v101 "$V101_AUTH" "$TMP/v101-index.json" "$V101_RESPONSE" \
  ...
  "$V101_CLOSURE" ''
run_case v110 "$V110_AUTH" "$TMP/v110-index.json" "$V110_RESPONSE" \
  "$V110_SIGNER" "$V110_SIGNER" .github/workflows/release-please.yml oci \
  "$V110_CLOSURE" ''
run_case v120 "$V120_AUTH" "$TMP/image-index.json" "$V120_RESPONSE" \
  ...
  "$V120_PROOF"
run_case v130 "$V130_AUTH" "$TMP/v130/image-index.json" "$V130_RESPONSE" \
  "$V130_SIGNER" "$V130_SIGNER" .github/workflows/release-please.yml api '' \
  "$V130_PROOF"
...
echo "image attestation verifier fixtures passed: closed v1.0.1/v1.1.0/v1.2.0/v1.3.0 authorities, candidate authority, exact gh argv, and atomic mutation rejection"
