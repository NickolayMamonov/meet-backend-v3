...
verify_environment_matches_container "$root" "$compose_script" &&
  ...
  fail "container environment differs"

zero_state_probe=
if [ -n "$public_url" ]; then
  zero_state_probe="$state_dir/zero-state-$phase.json"
  "$script_dir/probe-test-vps-zero-state.sh" \
    --phase "$phase" --root "$root" --compose-script "$compose_script" \
...
if [ -n "$zero_state_probe" ]; then
  probe_json=$(cat "$zero_state_probe")
  assets_count=$(jq -er '.http.assetsCount' "$zero_state_probe")
  assets_verified=$(jq -er '.http.assetsVerified' "$zero_state_probe")
  admin_key_configured=$(jq -er '.http.adminKeyConfigured' "$zero_state_probe")
...
jq -cnS --arg phase "$phase" --arg image "$expected_image" --arg imageId "$expected_image_id" \
  ...
  --argjson zeroStateProbe "$probe_json" \
  --argjson assetsCount "$assets_count" \
  --argjson assetsVerified "$assets_verified" \
